...
create table cm_membership_plans (
  ...
  price_cents                   int          not null comment 'EUR price in cents',
  currency                      varchar(3)   not null default 'EUR' comment 'Primary price currency; currently EUR',
  cny_price_cents               int          not null comment 'CNY price in fen',
  billing_type                  varchar(20)  not null comment 'free, one_time, recurring',
  billing_period                varchar(20)  default null comment 'monthly, quarterly, yearly',
  validity_months               int          default null comment 'Validity months for one-time plans',
  ...
  event_quota                   int          not null default 0 comment 'Event quota for one membership validity period',
...
create table cm_user_entitlement_balances (
  ...
  entitlement_code   varchar(50) not null comment 'private_introduction, event_registration, event_priority, staff_review, profile_detail_access',
...
create table cm_events (
  ...
  consumes_membership_quota tinyint(1) not null default 0 comment 'Whether confirmed registration consumes event_registration quota',
  city_code             varchar(80)  not null comment 'City code for filtering',
...
create table cm_event_registrations (
  ...
  event_quota_consumed_at datetime default null comment 'Time when event quota was consumed',
  event_quota_released_at datetime default null comment 'Time when consumed event quota was restored',
  created_at         datetime    not null default current_timestamp comment 'Create time',
  updated_at         datetime    not null default current_timestamp on update current_timestamp comment 'Update time',
  primary key (id),
