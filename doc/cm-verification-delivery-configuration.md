...
验证码生成并写入 Redis 后，投递任务会提交到独立的后台线程池，接口无需等待外部服务完成发送。前端收到成功响应后即可开始倒计时。

后台任务按请求中的 `provider` 投递：
...
验证码会先写入 Redis，再将外部投递提交到有界线程池。如果渠道不可用或任务无法入队，接口直接返回失败；任务成功入队后，接口立即返回成功。
...
清理操作会核对本次验证码 ID，避免误删后续重新生成的验证码。因此用户可以在外部服务恢复后重新请求，不会因为失败投递而持有一个无法收到的有效验证码，也不会被错误冷却。

接口返回成功表示“投递任务已受理”，不表示收件服务商已经最终接收邮件或短信。SMTP 接受后的异步退信需要通过供应商控制台或 Webhook 追踪。
