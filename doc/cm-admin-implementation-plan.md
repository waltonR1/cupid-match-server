...
下一任务是 Phase 8.1，严格按以下顺序执行：

1. 完成 `cupid-match-admin` 品牌清理。
2. 重做固定路由 `/index` 对应的运营首页。
3. 创建 `sql/cm_admin_menu.sql` 的角色、ID 分配和清理框架。
4. 建立 Cupid 角色，但不提前插入不存在页面的 `C` 菜单。
5. 验证超级管理员、普通 Cupid 角色和无 Cupid 权限用户的差异。
6. 完成登录、首页、角色和动态路由烟雾测试。
7. 再进入 Phase 8.2，并随真实页面加入审核中心菜单。
