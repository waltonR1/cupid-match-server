...
| 阶段八：RuoYi 后台运营 | 已完成并验收 | 后台运营、审核、通知、跟进事项、会员状态和治理能力已补齐 |
| 阶段九：支付、审计与生产化 | 非支付项已基本完成；支付线待做 | 安全、监控、热配置、Quartz、提醒、重试、清理和验证码真实投递已收口；支付、订单、会员正式开通与权益发放仍待外部协调 |
...
- Email/SMS 验证码真实通道（已接入 SMTP 与 Twilio，并完成真实发送验证）。
- 本地上传迁移至对象存储。
- Stripe 或人工确认支付流程。
- 数据库迁移工具和版本管理。
- 数据备份、恢复演练和敏感配置管理。
